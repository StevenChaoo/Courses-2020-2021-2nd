...
\begin{abstract}
    ...
    \zihao{-4}\fangsong 这样的工作往往是序列的标注工作，讲文本当作长序列，顺序学习提取出命名实体和他们之间的关系。
    \zihao{-4}\fangsong 本文在图论的基础上，提出了一个重构的多视图图模型用来捕捉命名实体之间的关系。
    \zihao{-4}\fangsong 之后通过调整图结构来选择出对于关系抽取工作来说最重要的字词，最后通过在序列上使用BERT模型，在结构上使用基于图论的图模型来共同抽取关系。
    \zihao{-4}\fangsong 通过实验验证，本文提出的方法在对话级别的关系抽取中有比较好的效果。\\
...
\begin{abstract}
    ...
    \zihao{-4} Such work is often sequence labeling work.
    \zihao{-4} The text is treated as a long sequence, and sequential learning is used to extract named entities and their relationships.
    \zihao{-4} Based on graph theory, this paper proposes a reconstructed multi-view graph model to capture the relationship between named entities.
    \zihao{-4} Afterwards, the graph structure is adjusted to select the most important words for the relationship extraction work, and finally the relationship is jointly extracted by using the BERT model in the sequence and the graph model based on graph theory in the structure.
    \zihao{-4} Through experimental verification, the method proposed in this paper has a relatively good effect in the extraction of relations at the dialogue level.\\
...
\songti\zihao{-4}也正是因为这个原因，由于句法书的错误提取，可能会导致错误传播的问题发生。

\songti\zihao{-4}本文提出了一个基于图论的关系抽取模型用来抽取对话级别的实体关系。
\songti\zihao{-4}在没有外部知识的情况下，构造一个多视图的图结构模型高度依赖人工提取的特征的有效性。
\songti\zihao{-4}这个图结构以文本中所有的命名实体为节点，基于BERT模型求出节点的表征向量，再根据某种规则对这些向量进行有向连接形成边。
\songti\zihao{-4}最后通过图卷积和图池化来逐一剪枝，最后确定本文中的关系。

\songti\zihao{-4}在学习潜在图的时候，两个命名实体之间的边往往记录了他们从文本序列中抽取出来时涵盖着的关系。
\songti\zihao{-4}这份关系往往不是对称的，所以我们需要生成有向图来初始化我们的图结构。
\songti\zihao{-4}具体来说，本文使用了高斯图生成器（GGG）\cite{Tarjan-1976}\cite{Bagdanov-2008}来初始化多视图图结构的边。
\songti\zihao{-4}然后通过测量不同节点的高斯分布之间的Kullback-Leibler（KL）散度\cite{Jognson-2001}来计算边节点的表征向量。
\songti\zihao{-4}由于KL散度的不对称性，GGG生成的图天然是有向图。
\songti\zihao{-4}在序列较长的情况下，初始化后的潜在图往往非常大，所以需要通过迭代图卷积和图池化来剪枝缩小规模。
\songti\zihao{-4}从而得到一个具有自适应能力的任务特定的图结构来进行本文关系抽取工作。
...
\songti\zihao{-4}本文基于这一思想，尝试生成多视图的图结构，用来模拟不同节点之间所有的结构可能。
\songti\zihao{-4}除此之外，本文还尝试提取出每一个图中的重要信息，来验证图结构的合理性。
...
\subsection{\songti\zihao{-4}高斯图模块}

\songti\zihao{-4}从BERT模型中产生的输出表示会被分为两部分，其中$h_0$作为一句话的句子级特征，代表了这句话归属于什么样的任务，这是第一部分。
\songti\zihao{-4}另一部分是剩下的隐层表示$H^{'}=\{h_1,h_2,\cdots,h_T,h_{T+1}\}$。在图结构部分，本文只用第二部分生成多视图的图模型。

\songti\zihao{-4}首先，将最初始的节点向量表示为$V^0=\{v_1^0,v_2^0,\cdots,v_{T+1}^0\}$，每一个节点都对应着原文本序列中的一个字/词。
\songti\zihao{-4}之后，使用高斯图生成器对多视图的图模型在给定节点集合的情况下进行边初始化。
\songti\zihao{-4}对于同一批节点来说，高斯分布的参数$\mu$和$\sigma$分别表示为：
\begin{align}
    \{\mu_i^1,\mu_i^2,\cdots,\mu_i^N\}&=g_{\theta}(v_i^0)\\
    \{\sigma_i^1,\sigma_i^2,\cdots,\sigma_i^N\}&=\phi(g_{\theta}^{'}(v_i^0))
\end{align}
\songti\zihao{-4}其中，$g_{\theta}$和$g_{\theta}^{'}$是两个神经网络中需要训练的参数，$\phi$是一个激活函数，激活函数是用来将神经网络中线性的变换转换为非线性的一种特殊机制。
\songti\zihao{-4}$N$代表了多视图图模型中视图的数目。
\songti\zihao{-4}本文将激活函数$\phi$设置为SoftPlus函数，因为高斯分布的标准差的值域是$(0,\infty)$。
\songti\zihao{-4}因此，我们可以得到对于第$n$个视图的一系列高斯分布$\{\mathcal{N}_1^n,\mathcal{N}_2^n,\cdots,\mathcal{N}_{T+1}^n,\}$。
\songti\zihao{-4}每一个高斯分布都是期望为$\mu_i^n$，方差为${\sigma_i^n}^2$的分布函数，表示为$\mathcal{N}_i^n(\mu_i^n,{\sigma_i^n}^2)$。

\songti\zihao{-4}多视图图模型是用来捕捉全部的关系可能的，所以本文在每一个字/词之间都使用了信息传播方法。
\songti\zihao{-4}本文使用了KL散度来计算每两个字/词之间的高斯分布差异度，根据这个差异度来生成边的权重关系。
\songti\zihao{-4}对于第$i^{th}$和第$j^{th}$个节点来说，在第$n^{th}$个视图中，KL散度的计算公式如下：
\begin{equation}
    e_{ij}^n=\mathrm{KL}(\mathcal{N}_i^n(\mu_i^n,{\sigma_i^n}^2)||\mathcal{N}_j^n(\mu_j^n,{\sigma_j^n}^2))
\end{equation}

\songti\zihao{-4}在计算完每个视图的每条边之后，我们就得到了一组临边矩阵$\{A^1,A^2,\cdots,A^N\}$，每一个临边矩阵都代表了一个视图。
\songti\zihao{-4}因此，通过高斯图模块生成的图可以表示为$G=(V^0,A^1,A^2,\cdots,A^N)$。

\subsection{\songti\zihao{-4}图卷积模块}

\songti\zihao{-4}本文在前人工作的基础上\cite{Guo-2019}\cite{Guo1-2019}，探索了多视图训练工作的方法。
...
\songti\zihao{-4}卷积层和池化层在神经网络中往往成对出现并不断循环，本文选用的是SAGPool池化模型\cite{Lee-2019}，计算公式如下：
\begin{equation}
...
\songti\zihao{-4}本文提出的基于图论的关系抽取模型可以应用于句子级和对话级的关系抽取任务。
\songti\zihao{-4}由于选用数据集的数据格式以及适用的基线模型是不同的，本文进行了两组实验来对模型的有效性进行了验证。
...
\songti\zihao{-4}文本同时还针对CNN模型，基于LSTM的RNN模型分别进行了实验，以保证实验的完整性。

\songti\zihao{-4}在试验参数的设置上，本文采用了相同的输入数据。
...
\songti\zihao{-4}除此之外，优化器我们选择了Adam\cite{Kingma-2015}。
\songti\zihao{-4}学习率设置为$3e-5$，池化率设置为$0.7$.我们使用了常见的SAGPool池化模型\cite{Lee-2019}作为本文的池化层。
\songti\zihao{-4}由于池化层的不断累加，最终图中的节点数量很少，采用传统的$F1$值作为评价指标不具备普适性，所以我们采用了$F1c$\cite{Yu-2020}指标作为模型试验的评价指标。
\songti\zihao{-4}输入上本文每批次仅将一组对话作为输入，而不是全部对话记录。
...
\songti\zihao{-4}基于这个想法，本文提出的基于图论的关系抽取模型是在融入了对话人身份信息的BERT模型的基础上进行的扩展。
\songti\zihao{-4}实验结果表明，本文提出的方法比BERTs的效果，在F1值上高5.9\%，在F1c值上高1.7\%。
...
\begin{table}[H]
	...
	\begin{tabular}{p{5cm}p{2cm}<{\centering}p{2cm}<{\centering}}
        ...
        Our Model & \textbf{78.9}\% & \textbf{67.5}\%\\
		\bottomrule[2pt]
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{clll}
		\toprule[2pt]
		\textbf{S1}: & \multicolumn{3}{l}{Hey, you guys! Look \textbf{what} I found! Look at this! \textbf{That's my} Mom's writing!}\\
        & \multicolumn{3}{l}{Look.}\\
        \textbf{S2}: & \multicolumn{3}{l}{Me \textbf{and Frank} and Phoebe, Graduation 1965.}\\
        \textbf{S1}: & \multicolumn{3}{l}{Y'know what that \textbf{means}?}\\
        \textbf{S2}: & \multicolumn{3}{l}{That you're actually 50?}\\
        \textbf{S1}: & \multicolumn{3}{l}{No-no, that's \textbf{not}, that's not me Phoebe, \textbf{that's} her pal Phoebe. According to}\\
        & \multicolumn{3}{l}{\textbf{her high school} yearbook, they were like B.F.F. Best \textbf{Friends} Forever.}\\
		\midrule[1pt]
		& \textbf{Argument Pair} & \textbf{Trigger} & \textbf{Relation type}\\
        \textbf{R1} & (S1, Frank) & high school & per:alumni\\
		\bottomrule[2pt]
	\end{tabular}
    \caption{\fangsong DialogRE测试数据集示例}
\end{table}

\songti\zihao{-4}同时，根据表3我们可以发现，通过本文提出的模型，在DialogRE数据集上选取的这一对话，我们可以选择出对关系抽取有用的触发词，如：high school。
\songti\zihao{-4}通过逐步减少对关系抽取无用的实体，使得模型在提取关系时有了更高的准确率。
...
\songti\zihao{-4}特别的，本文提出的模型包含有预训练语言模块——BERT模块、图结构模块——高斯图模块和图卷积模块。
\songti\zihao{-4}通过高斯图模块和图卷积模块，模型可以很好的模拟长序列中不同实体之间的可能的关系并做卷积学习。
